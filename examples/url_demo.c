...
int main() {
    ...
    pfxr_sound_t* sound_from_url = pfxr_create_params_from_url(example_url);
    if (sound_from_url) {
    ...
    char* url_from_sound = pfxr_get_url_from_params(&laser_sound);
    if (url_from_sound) {
    ...
    // Convert to URL
    char* jump_url = pfxr_get_url_from_params(&jump_sound);
    if (jump_url) {
        ...
        pfxr_sound_t* restored_jump = pfxr_create_params_from_url(full_url);
        if (restored_jump) {
    ...
    pfxr_sound_t* invalid_sound = pfxr_create_params_from_url("not-a-valid-url");
    ...
    pfxr_sound_t* no_fx_sound = pfxr_create_params_from_url("https://example.com?other=param");
